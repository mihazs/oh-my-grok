...
name: handoff
description: >
  Create a detailed HANDOFF CONTEXT summary for continuing work in a new Grok session.
  Use when the user says /handoff, handoff, context too long, fresh session, compaction,
  or wants a paste-ready continuation prompt. Ported from oh-my-openagent /handoff.
user_invocable: true
---
...
1. **Session history** — use this conversation (Grok has no `session_read`; rely on the thread plus any artifacts you wrote)
2. **Todos** — current `TodoWrite` list; optionally Read `.omg/todos/<sessionId>.json` if it exists
3. **oh-my-grok state** — Read when present:
   - `.omg/boulder.json` and active `.omg/plans/*.md`
   - `.omg/ralph-loop.local.md` (active Ralph / ultrawork loop)
4. **Git** (when the workspace is a repo):
   ```bash
   git diff --stat HEAD~10..HEAD
   git status --porcelain
   ```
5. **AGENTS.md** — Read at workspace root if it exists (constraints only)

Suggested order: todos → `.omg/` files → git → AGENTS.md.
...
- What files were modified or discussed (git diff/stat + status)
...
PLUGIN STATE (.omg)
-------------------
- [Boulder, plans, ralph-loop, todo mirror — or "none"]
...
- No markdown headers with `#` inside the HANDOFF CONTEXT block (use the format above with dashes)
- No bold, italic, or code fences within the HANDOFF CONTEXT content
...
After generating the summary, **Write** a copy to `.omg/handoffs/handoff-<YYYYMMDD-HHMMSS>.md` (create the directory if needed) and tell the user the path.
...
1. Start a new Grok session (new chat or reload session)
